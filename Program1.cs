...
class Minimizer
{
    ...
    public double Tangents(double a, double b, double epsilon = 1e-5, int maxIterations = 1000)
    {
        ...
        for (int i = 0; i < maxIterations; i++)
        {
            ...
            // Делаем шаг по направлению производной
            double step = 0.1; // шаг можно подстроить
            x = x - step * d;

            // Если выходим за границы — возвращаемся внутрь
            if (x < a) x = a + epsilon;
            if (x > b) x = b - epsilon;
        }
...
class Program
{
    static void Main()
    {
        ...
        // Создаем объект, передавая лямбда-выражение (функцию)
